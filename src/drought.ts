...
function renderDrought(
  ...
  content.innerHTML = `
    ...
    <section class="overview-card" aria-labelledby="drought-gap-heading">
      <div class="card-heading">
        <div><h2 id="drought-gap-heading">The same comparison, in order</h2><p>One row for each drainage area that has a reservoir reading. Both dots in a row sit on the same 0 to 100 scale, each one at its own value, so which dot is on the left changes from row to row. The dot in the class colours is the share of land in ${dryness.label.toLowerCase()} (${dryness.code}) or worse. The dot in the storage colours is how full that area's reservoirs are. The line between them is the distance, and it is only a distance. The two shares divide by different things, one by land and one by reservoir capacity, so the site never states their difference as a number. The rows are ordered by that distance: the areas where the water dot sits furthest to the left of the dry dot lead the list, because that is dry ground with little banked to draw on.</p></div>
      </div>
