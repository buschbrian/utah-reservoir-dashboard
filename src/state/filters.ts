...
export const ALL_RESERVOIRS: FilterState = { storageClass: null, reporting: "all" };
...
export function isFiltered(state: FilterState): boolean {
  return state.storageClass !== null || state.reporting !== "all";
}
 ...
 */
export function describeFilter(state: FilterState, shown: number, total: number): string {
  ...
  return `Showing ${shown} of ${total} reservoirs: ${parts.join(", ")}. ` +
