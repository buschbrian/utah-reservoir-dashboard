/*
 * Browser smoke test for both dashboards.
 *
 ...
 *   - a popup opens with a trend chart in it
 *
 * Both pages pull their SDK from a CDN, so this needs network access.
 */
...
const PAGES = [
  { name: "ArcGIS Maps SDK", url: `http://127.0.0.1:${PORT}/index.html`, engine: "arcgis" },
  { name: "MapLibre GL JS", url: `http://127.0.0.1:${PORT}/maplibre/index.html`, engine: "maplibre" }
];
...
for (const page of PAGES) {
  ...
  try {
    ...
    check(ready.layers === 3, `${page.name}: ${ready.layers} layers, expected 3`);
    ...
    // module failed to load, this is empty on both pages.
...
console.log("\nBoth dashboards rendered cleanly.");
